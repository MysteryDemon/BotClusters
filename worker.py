...
def start_bot(cluster):
    ...
    with bot_lock:
        ...
        try:
            ...
            #command = (f"bash -c 'source {venv_dir / 'bin' / 'bash'} && {bot_file}'" if bot_file.suffix == ".sh" else f"{venv_dir / 'bin' / 'python3'} {bot_file}")
            command = (f"bash -c 'source {venv_dir}/bin/activate && bash {bot_file}'" if bot_file.suffix == ".sh" else f"{venv_dir / 'bin' / 'python3'} {bot_file}")
