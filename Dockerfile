...
RUN apt-get update && \
    apt-get install -y gcc python3-dev mediainfo wget libsm6 libxext6 libfontconfig1 libxrender1 libgl1-mesa-glx g++ make pv jq git supervisor && \
    arch=$(arch | sed s/aarch64/arm64/ | sed s/x86_64/64/) && \
